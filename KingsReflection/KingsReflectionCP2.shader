...
{   
    ...
    p.xz = fract(p.xz)-0.5;
    p.xz *= 1.;
    p.xz = mod(distancePoint.xz, 1.9)-.95;
    
    if(length(distancePoint.xz) > 5.)//(distancePoint.xz)
    {
    ...
    */
    ...
    //p = p + fract(p*.1)-.5 + vec3(0,0,0);
    //p.z = mod(distancePoint.z, 0.0) - 0.15;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec4 _LightPos = vec4(2.,4. + sin(T*2.),5.,8.);//orig 2 height + sin
    ...
    float fAO = smoothstep(.0,.2,hitRef);//pow(smoothstep(.0,0.4,hitRef),1.)
    sceneColors *= vec3(clamp(fAO,0.,1.));
