...
#define REFLECTION 0.8 //0. TO 1.
#define COLORS 0. //0. OR 1.
...
{   
    ...
    //p.y +=  length(sin(T) +distancePoint.xz);
    ...
    if(floor(length(distancePoint.xz)) > 12. || (length(distancePoint.y)) > 8.  )//(distancePoint.xz)
    {
        return min(dSphere2,min(dSphere, min(fs,dPlane)));
    }
    
    if((q.x > 0. && q.x < 2.)&&(q.z < 4. && q.z > 2.)){p*=0.9;}
...
vec3 getObjectColor(vec3 p){
    ...
    vec3 index = (mod(gridId.x+gridId.y,2.) > 0.) ? vec3(0.93,0.93,0.82)*2.:vec3(0.46,0.58,0.33)*2.;//1. : 0.;
    
    if(p.y>0.1){
        if((p.x > 0. && p.x < 2.)&&(p.z < 4. && p.z > 2.)){return vec3(10,10,0);}
        //return vec3( hash12(floor(p.xz))*2.,hash12(floor(p.zx))*1.6,hash12(floor(p.xx))*3.4 );
        

        return (1. - COLORS) + (0. + COLORS)*
...
vec3 ColorScene(in vec3 hitEP,in vec3 rd,in vec3 n,in vec4 lp, float t)
{
    ...
    vec3 sceneColors = ((objCol*(diffuse+0.1)*(1.*dotNL*0.5+0.5)) + (vec3(1., 1., .2) * specular )) * (atten*1.);
    
    float depth = smoothstep(0.,1.,t*0.01);
    

    //vec3 p = palette(length(1.*PIHALF)  + iTime*5.,vec3(0.7, 0.5, 0.5),vec3(0.5, 0.2, 0.9),vec3(1.0, 0.5, 0.3),vec3(0.09, 0.33, 0.67));
    //return sceneColors;
    //return vec3(depth);
    return vec3(mix(sceneColors, vec3(.1,0.1,.3) ,depth));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if(rot == 1)
    {
        ...
        ro.z += -6. - sin(T)*1.2;
        rd.yz = vec2(rot2D(rd.yz,(PI*0.25)*s01(1.2)));
        ro.xz = vec2(rot2D(ro.xz,T));
        //rd.xz += vec2(rot2D(rd.xz,(sin(T*1.5)*0.25 * (1. - clamp(iMouse.z,0.,1.)) ) + ( clamp(iMouse.z,0.,1.) * (3.14 - iMouse.x*0.01)) ));//sin(T*1.2)*0.2));
        rd.xz = vec2(rot2D(rd.xz,T));
    ...
    vec4 _LightPos = vec4(1,4. + (sin(T*3.)*2.),3,8.);//orig 2 height + sin
    ...
    sceneColors += ColorScene(refHitPoint,rd,normalsHit,_LightPos,hitRef) * (REFLECTION);
